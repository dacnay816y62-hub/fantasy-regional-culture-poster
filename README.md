...
面向中国地域文化海报的 Codex Skill。它把省份、城市、县域、古城或文化区域转译成“地域文化机制 + 字体/图像关系 + 克制版式系统”的当代视觉海报。

核心目标不是做旅游明信片，也不是堆地标，而是让地域文化变成可识别的结构、材料、空间、气候和字形关系。

## v0.1.2 更新

- 移除公开文档中的本机输出路径、个人环境假设和特定模型硬依赖。
- 将“直接生成”改为通用能力描述：可使用任意支持图像生成的宿主工具或模型。
- 明确示例图片的权利状态：示例仅用于展示视觉方向，正式商用前请替换为自有或已授权资产。
- 保留 MODE B 的城市图像优先、边界克制、标题字体区域化、批量去重矩阵等规则。
